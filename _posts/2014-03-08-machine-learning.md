...
Задача имеет n features (параметров, но это слово уже занято для \\(\theta_j\\)
и m наборов данных в тренировочном наборе.
...
Выбираем некое начальное значение параметров. Вычисляем в данной точке значение 

Одновременно вычисляем новые значения для всех \\(\theta_j\\):
...
<div>
{% raw %}
\[ \theta_j = \theta_j - \alpha \frac{1}{m} \sum_{i=m}^{m} \left ( h_\theta (x^{(i)}) - y^{(i)} \right ) x_j^{(i)} \]
{% endraw %}
</div>
